...
const AddSeance = ({ data, dataForSeance, onDataSeancesAdd }) => {
  ...
  useEffect(() => {
    const selectedHallId = dataForSeance
      ? data?.halls?.find((hall) => hall.hall_name === dataForSeance.hall)?.id
      : "";
    const selectedFilmId = dataForSeance
      ? data?.films?.find((film) => film.film_name === dataForSeance.film)?.id
      : "";
      ...
      <section className="popup__container">
        <div className="popup__content">
          ...
          <form
            ...
            onSubmit={(event) => onAddSeance(event, onDataSeancesAdd, data)}
          >
            <div className="popup__settings">
              <label
                className="admin__label popup__label"
                htmlFor="seans-hall-name"
              >
                Название зала
              </label>
              <select
